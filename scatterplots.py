...
# import reader
from reader import loadAllFeedsFromFile, getAllTags, smallDict
from topicmap import getDocList, deriveTopicMaps,updateDictionaryByFuzzyRelevanceofTopics, getCustomStopWords
...
def displayTags(allItemDict, displayAmount=30):
    ...
    simpleTagDisplay(ax,tagnames,tagNumbers)
    ...
    sns.scatterplot(data=df2,x="Feeds", y="Tags", size="Number", ax=ax2,
...
def testDisplayTags(numArticles=300, numTopics=30, dict=None):
    if not dict:
        dict=loadAllFeedsFromFile()
    small=smallDict(dict,numArticles)
    displayTags(small,numTopics)
    return
